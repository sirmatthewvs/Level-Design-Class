...
namespace Gamekit3D.Cameras
{
    ...
    public class LayerCullDistances : MonoBehaviour
    {
        ...
        int m_PreviousQualitySetting = -1;
        ...
        void Update()
        {
            // Avoid doing expensive work every editor frame. Recompute only when needed.
            if (!Application.isPlaying)
                return;

            int q = QualitySettings.GetQualityLevel();
            if (q != m_PreviousQualitySetting)
            {
                m_PreviousQualitySetting = q;
                ComputeLayerCullDistances();
            }
        }
    }
